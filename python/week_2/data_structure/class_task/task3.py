# Task3
# Given an 'apple,banana,orange", split the string into a list of fruits.
fruits = "apple,banana,orange" 
print(fruits.split(","))

# Ask the user for a sentence and print each word in a new line
user = input("Write a Sentence: ")
print("\n".join(user))

# Replace all spaces in a string with underscores
message = "I love Programing"
print(message.replace (" ", "_"))

# Count how many times the letter "a" appears in "Banana".
text = "Banana"
print(text.count("a"))

# Check if a given string starts with "https://"
text = "https://www.Abeokuta.com"
print(text.startswith("https://"))